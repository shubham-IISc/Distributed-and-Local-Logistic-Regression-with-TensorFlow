...
if FLAGS.job_name=='ps':
    ...
    with tf.device(tf.train.replica_device_setter(worker_device="/job:worker/task:%d" % FLAGS.task_index,cluster=cluster)):
        ...
        replicated_opt=tf.train.SyncReplicasOptimizer(opt=optimizer,total_num_replicas=len(workers),replicas_to_aggregate=len(workers),use_locking=True)
    ...
    with sv.prepare_or_wait_for_session(server.target) as session:
        if FLAGS.task_index == 0:
            sv.start_queue_runners(session, [queue_runner])
